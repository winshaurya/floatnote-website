...
export const LaserFlow = ({
  ...
  // Throttle rendering to save CPU/GPU when not necessary
  const TARGET_FPS = 40; // render target (can be tuned down to 30)
  const MIN_FRAME_TIME = 1 / TARGET_FPS; // seconds
  let lastRenderTime = 0;
    ...
    // allow external triggers (parent can dispatch 'laser-resize' when it repositions the wrapper)
    window.addEventListener('laser-resize', setSizeNow, { passive: true });
    ...
    // Keep rectRef up-to-date during scroll/swipe/orientation changes so pointer mapping doesn't drift.
    const updateRect = () => {
      rectRef.current = canvas.getBoundingClientRect();
    };
    // Update rect on scroll/touchmove and re-run a resize on orientation changes.
    window.addEventListener('scroll', updateRect, { passive: true });
    window.addEventListener('touchmove', updateRect, { passive: true });
    const onOrientation = () => scheduleResize();
    ...
    const animate = () => {
      ...
      // Update uniform times continuously but only render at target FPS
      ...
      // Throttle actual GPU renders
      if (t - lastRenderTime >= MIN_FRAME_TIME) {
    ...
    return () => {
      ...
      // remove window listeners we added to keep rect up-to-date on scroll/swipe/orientation
      window.removeEventListener('scroll', updateRect);
      window.removeEventListener('touchmove', updateRect);
      window.removeEventListener('orientationchange', onOrientation);
      // remove external resize trigger
      window.removeEventListener('laser-resize', setSizeNow);
  ...
  // Default beam color should be the provided color; fallback to white
